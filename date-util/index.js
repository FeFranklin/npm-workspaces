import StringUtil from '@fefranklin/string-util'
...
export default class DateUtil {
  ...
  static formatString(dateStr, currentFormat, expectedFormat) {
    if (StringUtil.isEmpty(dateStr)) {
      return { error: 'your txt is empty'}
    }
    ...
    const dateStrInSO = StringUtil.removeEmptySpaces(dateStr).replace(toDateExp, '$<year>-$<month>-$<day>')

    const finalDate = new Date(dateStrInSO)
